...
void searchPatientByPhoneNumber(const struct Patient patient[], int max) {
    ...
    for (ind = 0; ind < max + 1; ind++) {
        if (strcmp(patient[ind].phone.number, phoneNumber) == 0) {
            printf("%05d %-15s ", patient[ind].patientNumber, patient[ind].name);
            displayFormattedPhone(patient[ind].phone.number);
            printf(" (%s)\n", patient[ind].phone.description);
            match++;
        }
    }
    printf("\n");
...
int nextPatientNumber(const struct Patient patient[], int max) {
    int nextNum, maxNum = patient[0].patientNumber, ind;

    for (ind = 0; ind < max; ind++) {
        if (patient[ind].patientNumber > maxNum) {
            maxNum = patient[ind].patientNumber;
        }
    }
...
int findPatientIndexByPatientNum(int patientNumber, const struct Patient patient[], int max) {
    ...
    for (ind = 0; ind < max; ind++) {
        if (patient[ind].patientNumber == patientNumber) {
            return ind;
        }
    }
...
void inputPatient(struct Patient* patient) {
    ...
    for (ind = 0; ind < strlen(name) - 1; ind++) {
        patient->name[ind] = name[ind];
    }
...
void inputPhoneData(struct Phone* phone) {
    ...
    case 1:
        ...
        for (ind = 0; ind < PHONE_LEN + 1; ind++) {
            phone->number[ind] = inputNumber[ind];
        }
